...
class Major:
    ...
    async def main(self):
        while True:
            try:
                ...
                logger.info(f"main | Thread {self.thread} | {self.name} | CYCLE COMPLETED")
    ...
    async def play_swipe_coin(self):
        ...
        if 'detail' in await resp.json():
            ...
            logger.success(f"swipe_coin | Thread {self.thread} | {self.name} | collected {json_data['coins']} stars")
    ...
    async def play_roulette(self):
        ...
        if 'detail' in await resp.json():
            ...
            logger.success(f"roulette | Thread {self.thread} | {self.name} | collected {(await response.json())['rating_award']} stars")
        return await response.json()
    ...
    async def play_hold_coin(self):
        ...
        if 'detail' in await resp.json():
            ...
            logger.success(f"hold_coin | Thread {self.thread} | {self.name} | collected {json_data['coins']} stars")
        return await response.json()
    ...
    async def join_squad(self):
        ...
        if (await resp.json())['status'] == 'ok':
            logger.success(f"join_squad | Thread {self.thread} | {self.name} | Joined squad")
